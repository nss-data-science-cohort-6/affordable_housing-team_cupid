...
```{r}
predict(simple_regression_model_log, newdata = prop_sold_5yrs)
```


```{r}
predict(simple_regression_model_log, prop_sold_5yrs, interval = c("confidence"), level = 0.95) %>% 
  head()
```
...
prop_sold_5yrs %>%
  ggplot(aes(
    x = ownerdate, 
    y = amount, 
    color = group)) + scale_y_log10() + geom_point() + facet_wrap(~ prox)
```
```{r}
