...
pd_within_mi_sales <- pd_within_mi_sales %>%
  mutate(group = 
           case_when(
             ...
             dist > 0.5 & difference >= -5 & difference <= 5 ~ "outside",
             TRUE ~ "other"
    )
  )
```

#### Add additional grouping "near" and "far" to segment groups
```{r}
pd_within_mi_sales <- pd_within_mi_sales %>%
  mutate(prox = 
           case_when(
...
```{r}
prop_sold_5yrs_no_geo <- prop_sold_5yrs %>% 
  st_drop_geometry(NULL) %>% 
  as.tibble()
write.csv(prop_sold_5yrs_no_geo, "prop_sold_5yrs_no_geo.csv", row.names=FALSE)
```
...
```{r}
simple_regression_model_log <- lm(log(amount) ~ square_footage +  age + group + ownerdate + tract + land_area_acres, data = prop_sold_5yrs)
...
```{r}
prop_sold_5yrs %>% 
  ggplot(aes(x = dist, y = amount)) + geom_point() + scale_y_log10() + facet_wrap(~ group)
```

```{r}
ggplot(prop_sold_5yrs, aes(x = amount)) + geom_histogram() + facet_wrap(~ group)
```
...
```{r}
prop_sold_5yrs %>% 
  ggplot(aes(x = amount, y = square_footage, color = group)) + scale_x_log10() + scale_y_log10() + geom_point() + facet_wrap(~ group)
```
```{r}
prop_sold_5yrs %>% 
  ggplot(aes(x = age, y = amount, color = group)) + scale_y_log10() + geom_point() + facet_wrap(~ group)
```

```{r}
prop_sold_5yrs %>% 
  ggplot(aes(x = land_area_acres, y = amount, color = group)) + scale_x_log10() + scale_y_log10() + geom_point() + facet_wrap(~ group)
```

```{r}
prop_sold_5yrs %>%
  filter(PROJECT == "Gibson Creek") %>% 
  ggplot(aes(y = amount, color = group)) + scale_y_log10() + geom_boxplot() + facet_wrap(~ prox)
```
...
prop_sold_5yrs %>%
  ...
  ggplot(aes(
    x = ownerdate, 
    y = amount, 
    color = group)) + scale_y_log10() + geom_point() + facet_wrap(~ prox) + geom_smooth(method = lm)
```
```{r}
saveRDS(prop_sold_5yrs, file = "prop_sold_5yrs.RData")
```
